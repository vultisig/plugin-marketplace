...
import { Form, FormProps, Input, InputNumber, Modal, Select } from "antd";
...
export const PolicyForm: FC<PolicyFormProps> = ({ app, onClose, schema }) => {
  ...
  return (
      ...
      title={
        <HStack $style={{ gap: "8px" }}>
          <Stack
            as="img"
            src={app.logoUrl}
            $style={{ borderRadius: "6px", height: "24px", width: "24px" }}
          />
          <HStack
            $style={{
              fontSize: "22px",
    ...
    >
      <VStack $style={{ flex: "none", gap: "16px", width: "218px" }}>
        {[t("templates"), t("configuration"), t("rules"), t("scheduling")].map(
          (item, index) => {
